<x-app-layout>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200">
            ...
            <div class="p-6 border-b border-slate-200 bg-blue-50">
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    ...
                    <!-- Barcode Scanner for Quick Find -->
                    <div x-data="checkinScanner()" class="flex-shrink-0">
                        <x-barcode-scanner inputId="checkin-scan" />
                    </div>
                </div>
            </div>

            <!-- Scanned Item Highlight -->
            <div x-data="{ highlightedItem: null }" 
                @barcode-scanned.window="highlightItem($event.detail.barcode)"
                x-init="$watch('highlightedItem', (val) => { if(val) setTimeout(() => highlightedItem = null, 3000) })">
                
                <template x-if="highlightedItem">
                    <div class="p-4 bg-green-50 border-b border-green-200">
                        <div class="flex items-center gap-2 text-green-700">
                            ...
                            <span x-text="highlightedItem"></span>
                        </div>
                    </div>
                </template>
                ...
                <script>
                    function highlightItem(barcode) {
                        // Find item row by barcode
                        const rows = document.querySelectorAll('[data-barcode]');
                        let found = false;
                        rows.forEach(row => {
                            if (row.dataset.barcode === barcode || row.dataset.code === barcode) {
                                row.scrollIntoView({ behavior: 'smooth', block: 'center' });
                                row.classList.add('ring-2', 'ring-green-500', 'bg-green-50');
                                setTimeout(() => {
                                    row.classList.remove('ring-2', 'ring-green-500', 'bg-green-50');
                                }, 3000);
                                this.highlightedItem = row.dataset.name;
                                found = true;
                            }
                        });
                        if (!found) {
                ...
                <div class="overflow-x-auto">
                    <table class="w-full">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            data-barcode="{{ $reqItem->item->barcode }}"
                            data-code="{{ $reqItem->item->code }}"
                            data-name="{{ $reqItem->item->name }}"
                            class="transition-all duration-300">
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    <div class="text-sm text-slate-500">
                                        {{ $reqItem->item->code }} 
                                        @if($reqItem->item->barcode)
                                        <span class="ml-1 text-xs text-slate-400">({{ $reqItem->item->barcode }})</span>
                                        @endif
    ...
    <script>
        function checkinScanner() {
            return {
                // Additional scanner logic if needed
            }
        }
    </script>
</x-app-layout>
